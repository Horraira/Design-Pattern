...
def check_credentials(username, password, data, user_type):
    try:
...
def admin_login():
    try:
        admin_window = create_window("Administrator Login", width=700, height=400)
        admin_details_label = tk.Label(admin_window, text="Enter Administrator Details:", font=("Roboto", 16), bg="#3498db", fg="white")
        ...
        error_label.pack()
        login_button = tk.Button(admin_window, text="Login", font=("Roboto", 12, "bold"), bg="#2ecc71", fg="white", width=15,
                             command=lambda: switch_to_admin_panel(admin_window) if check_credentials(username_entry.get(), password_entry.get(), data, "administrators") else error_label.config(text="Invalid credentials"))
...
def switch_to_admin_panel(admin_window):
    try:
        ...
        admin_panel = create_window("Admin Panel", width=700, height=700)
        enter_user_details_label = tk.Label(admin_panel, text="Enter User Details:", font=("Roboto", 16), bg="#3498db", fg="white")
        ...
        action_label = tk.Label(admin_panel, text="What do you wish to do?", font=("Roboto", 16), bg="#3498db", fg="white")
        action_label.pack(pady=20)
        selection = tk.IntVar()
        create_radio = tk.Radiobutton(admin_panel, text="Create User Account", font=("Roboto", 14), bg="#3498db", variable=selection, value=1, cursor='arrow')
        create_radio.pack(pady=5)
        remove_radio = tk.Radiobutton(admin_panel, text="Remove User Account", font=("Roboto", 14), bg="#3498db", variable=selection, value=2, cursor='arrow')
        remove_radio.pack(pady=5)
        error2_label = tk.Label(admin_panel, text="", font=("Roboto", 10), bg="#3498db", fg="red")
        submit_button_admin_panel = tk.Button(admin_panel, text="Submit", font=("Roboto", 14), bg="#2ecc71", fg="white", command=lambda: submit_action(new_username_entry.get(), new_password_entry.get(), selection, error2_label, success_label))
...
def start_game(player1_username, player1_password, player2_username, 
    ...
    try:
        if (not player1_username or 
            ...
            return
        if (check_credentials(player1_username, player1_password, data, "players") and 
            check_credentials(player2_username, player2_password, data, "players")):
...
def switch_to_player_login():
    try:
        player_login_window = create_window("Player Login", width=700, height=600)
        ...
        if selection.get() == 1:  # Create User Account
            for player in data["players"]:
                if player["username"] == username:
                    ...
                    return
            data["players"].append({"username": username, "password": password})
    ...
    try:
        ...
        def show_winner(player):
            ...
            exit_button = tk.Button(winner_dialog, text="Exit", font=("Roboto", 12), 
                                    bg="#e74c3c", fg="white", command=lambda: close_windows(winner_dialog, connect4_window))
        ...
        def show_draw_message():
            ...
            exit_button = tk.Button(draw_dialog, text="Exit", font=("Roboto", 12), 
                                    bg="#e74c3c", fg="white", command=lambda: close_windows(draw_dialog, connect4_window))
            exit_button.pack(pady=10)

        def reset_board(dialog):
            for row in board:
                for circle in row:
                    connect4_board.itemconfig(circle, fill=circle_color)
...
try:
    ...
    data = read_data_file()
    # error_label = tk.Label(root, text="", font=("Roboto", 10), bg="#3498db", fg="red")
    # success_label = tk.Label(root, text="", font=("Roboto", 12), bg="#3498db", fg="green")
    ...
    admin_radio = tk.Radiobutton(root, text="Administrator", font=("Roboto", 14), bg="#3498db", variable=selection, value=1)
